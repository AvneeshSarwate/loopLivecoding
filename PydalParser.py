...
def isFloatSymbol(s):
    return re.match('^\.[0-9]+$', s) or s == "~" or s == "="
...
def parseParenBlock(tokenList, ind, isSymbol, parseDebug = False):
    ...
    if openParen == "[":
        ...
        node = nodes.ExpandBracketNode([])
    ind += 1

    while not isEndParen(tokenList[ind]):
        parseFunc = parseExpandExpression if openParen == "|" else parseExpression
        expNode, newInd = parseFunc(tokenList, ind, isSymbol)
...
def parseExpression(tokenList, ind, isSymbol):
    ...
    while ind < len(tokenList) and not isComma(tokenList[ind]) and not isEndParen(tokenList[ind]): #check this
        ...
        if isSymbol(tokenList[ind]):
            ...
            raise StopIteration("can only start expressions with symbols or open paren: " + str(ind)) 

        ind = newInd

    return node, ind

def parseExpandExpression(tokenList, ind, isSymbol):
